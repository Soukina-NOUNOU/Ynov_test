...
describe("RegistrationForm / complete integration", () => {
  ...
  test("Soould affiche une erreur pour un code postal invalide", async () => {
    ...
    expect(
      screen.getByText("Le code postal doit être composé de 5 chiffres exactement.")
    ).toBeInTheDocument();
  });
  ...
  test("Should save errors to localStorage when submitting with invalid postal code", async () => {
    ...
    expect(localStorage.getItem("error_postalCode")).toBe("Le code postal doit être composé de 5 chiffres exactement.");
  });
